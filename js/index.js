...
//objects to store the application's state
var gameState = {
  gameGrid: [[]],
...
function initGame() {
  //if there is a saved game in progress load that instead of generating a new game board
  if (localStorage.getItem("gameGridObject" + gameState.difficulty)){
    loadGameState();
  }  
  else {
    ...
    gameState.time = 0;
    gameState.flaggedSquareCounter = 0;
  ...
  setDynamicSizes();
  //resets the cheating flag for a new game
...
//sets the gameStates for difficulty, gridSize, and number of pigs depending on difficulty
...
function startTimer(restart) {
  ...
  function myTimer() {
    ...
    gameState.time = Math.round((currentTime - startTime) / 1000) + restartTime;
...
function clearAllGameStates(){
  ...
  localStorage.removeItem("gameGridObjectswineMedium");
  localStorage.removeItem("gameGridHtmlswineMedium");    
  localStorage.removeItem("gameGridObjectswineHard");
  localStorage.removeItem("gameGridHtmlswineHard");    
  localStorage.removeItem("gameGridObjectcustom");
  localStorage.removeItem("gameGridHtmlcustom");  
}
...
function winBehavior() {
  ...
  if(gameState.difficulty != "custom" && gameState.cheating !== true){
...
function plusPigNumber() { 
  ...
  var maxPigs = Math.pow(gameState.gridSize, 2) - 1;

  
  //increase the number of pigs as long as it doesn't exceed the maximum
  if (gameState.numPigs < maxPigs) {
    playSound('lightClickSound');
    gameState.numPigs++;
    $customNumPigs.html(gameState.numPigs);
  } 
  
  //if pigs reach the maximum but the gridSize is below its maximum increase the gridSize and pigs
  else if ((gameState.numPigs === maxPigs) && (gameState.gridSize < gameState.maxGrid)) {
    gameState.numPigs++;
    gameState.gridSize++;
    $customNumPigs.html(gameState.numPigs);
    $customGridSize.html(gameState.gridSize + "x" + gameState.gridSize);
  }
}
...
function plusGridNumber() { 
  ...
  if (gameState.gridSize < gameState.maxGrid) {    
    playSound('lightClickSound');
    gameState.gridSize++;
    $customGridSize.html(gameState.gridSize + "x" + gameState.gridSize);
  }
...
function minusGridNumber() { 
  ...
  if (gameState.gridSize > 2) {
    ...
    if (gameState.numPigs > maxPigs) {
      gameState.numPigs = maxPigs;
      $customNumPigs.html(gameState.numPigs);
    }
  }
}

function loadCustomGameSettings() {
  //first verify there are saved sound settings
  if (localStorage.getItem("customGameSettings")) {
    var settings = localStorage.getItem("customGameSettings");
    var customSettings = settings.split(",");
    
    gameState.customPigs = customSettings[0];
    gameState.customGrid = customSettings[1];
  }
}
  ...
  .on('vclick', function () {
    ...
    setTimeout(function(){
      if (gameState.difficulty === "custom") {
        //loads previous custom game settings, stays at default if there are none
...
$('.check')
  .on('vclick', function () {
    if(gameState.customGrid !== gameState.gridSize || gameState.customPigs !== gameState.numPigs){
      clearGameState();
    }
    //prevent clicks during transition
    $clickBlocker.show();
    //sets numPigs to custom selection
    gameState.customPigs = gameState.numPigs;
    //sets gridSize to custom selection
    gameState.customGrid = gameState.gridSize;
    ...
    localStorage.setItem("customGameSettings", [Number(gameState.numPigs), Number(gameState.gridSize)]);
  });
...
function loadSoundSettings() {
  ...
  if (localStorage.getItem('swinePeeperSound')) {
    ...
    soundState.playSounds        = soundSettings[0];
    soundState.playMusic         = soundSettings[1];
...
function clearSoundSettings(){
  localStorage.removeItem("swinePeeperSound");
}

//load the game's sound effects
function loadSounds() {
  createjs.Sound.registerSound("sound/buttonclick.mp3", 'clickSound', 10);
  createjs.Sound.registerSound("sound/clickisland.mp3", 'clickIslandSound', 10);
  createjs.Sound.registerSound("sound/lightclick.mp3", 'lightClickSound', 10);
  createjs.Sound.registerSound("sound/pokepig.mp3", 'pokePigSound', 10);
  createjs.Sound.registerSound("sound/clickblock.mp3", 'clickBlockSound', 10);
  createjs.Sound.registerSound("sound/flagblock.mp3", 'flagBlockSound', 10);
  createjs.Sound.registerSound("sound/pageflip.mp3", 'pageFlipSound', 10);
  createjs.Sound.registerSound("sound/sleepingpigs.mp3", 'sleepingPigSound', 1);
}
...
function startMusic() {
  ...
  if (soundState.musicInitialized && soundState.playMusic && soundState.playingMusic !== true) {
    $jPlayer.jPlayer('play');
    soundState.playingMusic = true;
  }
  
  //if music isn't muted but hasn't been played yet load the sound file
  else if (soundState.playMusic && soundState.playingMusic !== true) {
    soundState.musicInitialized = true;

    //load song and start playing it
    $jPlayer.jPlayer({
      ready: function (event) {
        $(this).jPlayer("setMedia", {
          mp3: "sound/panamahatsong.mp3",
        }).jPlayer('play');      
      },
      ...
      playing: function () {
        ...
        soundState.musicLoopOneActive = true;
        soundState.playingMusic = true;
      },
      supplied: "mp3",
      wmode: "window",
      ...
      playing: function () {
        musicLoopTwo = setTimeout(function(){
          if (gameState.playingGame){
            stopMusic();
            $jPlayer.jPlayer("play"); 
            soundState.musicLoopTwoActive = false;
          }       
        }, 102850);
        soundState.musicLoopTwoActive = true;
        soundState.playingMusic = true;
      },
...
function stopMusic() {
  ...
  if(soundState.musicLoopOneActive){
    clearTimeout(musicLoopOne);
    soundState.musicLoopOneActive = false;
  }
  if(soundState.musicLoopTwoActive){
    clearTimeout(musicLoopTwo);
    soundState.musicLoopTwoActive = false;
  } 
  soundState.playingMusic = false;
}
